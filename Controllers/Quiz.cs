...
public class Quiz : Controller
{
    ...
    public IActionResult Questoes(int? tema, int? nivel)
    {
        ...
        ViewBag.PerguntaX = _context.Perguntas
                ...
                .OrderBy(p => Guid.NewGuid()) // Ordena as perguntas por um número aleatório no lado do servidor
        ...
        ViewBag.ItemDaPergunta = _context.ItemDaPerguntas
            // ⚡ Bolt: Optimize query by selecting random rows on the DB side instead of loading everything into memory
            .OrderBy(p => Guid.NewGuid())// Ordena os itens da Pergunta por um número aleatório no lado do servidor
            .ToList();
